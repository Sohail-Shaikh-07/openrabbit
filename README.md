...
| Prompting | Changed-line evidence first, token-aware PR diff compression, low-risk large-file summaries, shared source budgets for RAG, connectors, memory, linked issues, and quality gates, strict JSON contract, no speculative findings |
...
When enabled, MCP, web search, multi-repo, Jira, and Linear connectors can add bounded, source-labeled context to `review`, `describe`, `ask`, and `improve`. Connector snippets are treated as untrusted evidence, scored against PR metadata, changed paths, changed symbols, linked issue keys, repository handles, and source kind, deduplicated across agent dimensions, packed under a connector-specific source budget, and surfaced in command summaries through connector counts, context provenance, and `context_diagnostics`. Weak connector matches are dropped with `weak_connector_relevance`, and over-budget connector candidates are dropped with `connector_item_limit`. `openrabbit eval` includes connector item totals, context candidate and selected counts, dropped reasons, prompt-packing estimates, and source counts in JSON, dashboard, and Markdown reports.
...
During review, OpenRabbit uses the changed file paths and changed symbols from the PR diff to prefer context from the files being edited, related test paths, nearby files, scoped guideline files, and architecture docs before broader semantic matches. Retrieved context is packed deterministically so direct evidence stays ahead of general repository context.
...
JSON output from model-facing commands includes `context_diagnostics` with local-only candidate counts, selected source counts, dropped reasons, score summaries, connector availability counts, source budgets, and estimated prompt-packing size. The shared packing defaults keep changed-line evidence and compressed diff evidence first, then bound repository RAG, connector snippets, PR memory, linked GitHub issues, and local quality diagnostics under separate budgets so one noisy source cannot crowd out the others. Oversized low-risk files such as generated docs or lock-style artifacts are represented as visibly marked summaries in compressed diffs, and diagnostics count `large_low_risk_files`, `large_low_risk_changes`, and `large_low_risk_diff_lines`. These diagnostics are intended for troubleshooting and eval dashboards; they do not include raw tokens, credentials, or unbounded connector output. See [docs/context-precision.md](docs/context-precision.md) for retrieval reasons, source budgets, connector relevance, eval interpretation, and missing or noisy context troubleshooting.
