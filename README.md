...
The Jira connector can read linked issue keys such as `SEC-42` when explicitly enabled with `base_url` and `token_env`. The token env can hold a raw bearer/PAT token or a full `Bearer ...` or `Basic ...` authorization value. Jira write mode stays off by default; when enabled, it is limited to one managed OpenRabbit summary comment on the linked issue. It does not create issues, transition status, assign users, change labels, or publish arbitrary Jira comments. Later v1.6 tasks will decide where retrieved connector snippets enter review prompts.
